...
def parse_args():
    parser = argparse.ArgumentParser(
        description="The fine tuned LlaMa generates data for evaluation."
    )
...
def main(args):
    ...
    if args.evaluation_type == 'effectiveness' or args.evaluation_type == 'security':
        Evaluation_gen.generate_code_for_effectiveness_and_security(llama, args.evaluation_path, args.openzeppelin_path)
    elif args.evaluation_type == 'correctness':
        Evaluation_gen.generate_code_for_correctness(llama, args.evaluation_path, args.openzeppelin_path, argparse.n)
    else:
